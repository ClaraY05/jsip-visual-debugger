(** Turns a perf report over the natively compiled, unmodified program into
    ...
    [bin/perf_heat.exe], which drives {!Heat_profile.of_perf_report}:
    {!Perf_report} parses the rows, {!Ocaml_symbol} decodes each symbol name,
    and {!Heat_profile} aggregates and writes the sexp contract file. *)
